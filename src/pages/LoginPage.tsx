...
const LoginPage = () => {
  ...
  const handleGoogle = async () => {
    setLoading(true);
    try { await loginWithGoogle(); toast.success("Welcome!"); navigate("/home"); }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Sign in to access your knowledge base</p>
        </div>

        <div className="rounded-xl border border-border bg-card p-6 shadow-lg">
          <Button variant="outline" className="w-full mb-4 h-11 border-border" onClick={handleGoogle} disabled={loading}>
